...
\newcommand\CardFrontCommit[2][\centering\rede{git push!}]{% Text, Zahlwert
	\CalcNumberColor{#2}%
	\CardFrontTikzCircle{commit}{\DrawNumber}{numcolor}{#1}%
}
\newcommand\CardFrontCommitPlus[4][\centering\rede{git push!}]{% Text, Zahlwert, Ecksymbol, Eckfarbe
	\CalcNumberColor{#2}%
	\CardFrontTikzCircle{commit}{%
		\DrawNumber %
		\node[draw, line width=1pt, fill=#4, circle, anchor=north west] at (UL) {\bigfont{#3}};%
		\node[draw, line width=1.25pt, fill=#4, circle, anchor=north east] at (UR) {\bigfont{#3}};%
	}{numcolor}{#1}%
}
...
\CardRelease%
\CardRelease%
\CalcNumberColor{2}\CardFrontTikzCircle{rebase}{\DrawNumber}{magenta!65}{%
	\centerline{\rede{git rebase!}}\vskip1ex
	%todo%Mit diesem Commit verursachst und löst du einen Merge Conflict:\vskip0.5ex
	\Item Übernimm die vorherigen drei Commits als deine eigenen, leg die Karten unter diese. Alle diese Commits gelten ab jetzt als einer.}%
